...
resource "aws_route_table" "route-table-pub-1a" {
  vpc_id = "${aws_vpc.vpc.id}"
...
resource "aws_route_table_association" "route-table-pub-1a" {
  subnet_id      = "${aws_subnet.subnet-pub-1a.id}"
  route_table_id = "${aws_route_table.route-table-pub-1a.id}"
}
